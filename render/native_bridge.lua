...
local nativeBridge = {
  ...
  _logOnce = {
    ...
    render_with_shaders_ok = true,
    render_with_shaders_fail = true,
    catalog_fail = true
  }
}
...
local nativeConfigModule -- lazy-loaded render/native_config
local function getNativeConfig()
  if nativeConfigModule == nil then
    local ok, cfg = pcall(require, "render.native_config")
    if not ok then
      ok, cfg = pcall(require, "native_config")
    end
    if ok and type(cfg) == "table" then
      nativeConfigModule = cfg
    else
      nativeConfigModule = false
    end
  end
  return nativeConfigModule ~= false and nativeConfigModule or nil
end

local shaderStackModule -- lazy-loaded render/shader_stack
local function getShaderStack()
  if shaderStackModule == nil then
    local ok, stack = pcall(require, "render.shader_stack")
    if not ok then
      ok, stack = pcall(require, "shader_stack")
    end
    if ok and type(stack) == "table" then
      shaderStackModule = stack
    else
      shaderStackModule = false
    end
  end
  return shaderStackModule ~= false and shaderStackModule or nil
end
...
function nativeBridge.setForceDisabled(v)
  ...
  if nativeBridge._forceDisabled then
    nativeBridge._catalogCache = nil
  end
end
...
-- Native shader stack renderer (geometry + shader execution + rasterization)
-- When native mode is enabled, this bypasses the Lua shader pipeline entirely
-- and executes the configured shader stack inside the C++ module.
function nativeBridge.renderWithShaders(voxels, params, stackConfig)
  local nativeConfig = getNativeConfig()
  if not (nativeConfig and nativeConfig.forceNative) then
    return nil, "native disabled"
  end

  local m = mod()
  if not (m and m.render_with_shaders) then
    if not nativeBridge._logOnce.render_with_shaders_fail then
      nativeBridge._logOnce.render_with_shaders_fail = true
      print("[asevoxel-native] render_with_shaders not available (falling back)")
    end
    return nil, "native missing"
  end

  local cfg = stackConfig
  if cfg == nil then
    if params and type(params.shaderStack) == "table" then
      cfg = params.shaderStack
    else
      local shaderStack = getShaderStack()
      if shaderStack and shaderStack.prepareStackConfig then
        cfg = shaderStack.prepareStackConfig(params or {})
      end
    end
  end
  ...
  if type(cfg.lighting) ~= "table" then cfg.lighting = {} end
  if type(cfg.fx) ~= "table" then cfg.fx = {} end

  local ok, result = pcall(m.render_with_shaders, voxels, params or {}, cfg)
  if not ok or type(result) ~= "table" or type(result.pixels) ~= "string" then
    if not nativeBridge._logOnce.render_with_shaders_fail then
      nativeBridge._logOnce.render_with_shaders_fail = true
      print("[asevoxel-native] render_with_shaders FAILED, falling back: " .. tostring(result))
    end
    return nil, result
  end

  if not nativeBridge._logOnce.render_with_shaders_ok then
    nativeBridge._logOnce.render_with_shaders_ok = true
    print("[asevoxel-native] render_with_shaders (native)")
  end

  return result
end

function nativeBridge.getNativeShaderCatalog(forceRefresh)
  if not forceRefresh and nativeBridge._catalogCache then
    return nativeBridge._catalogCache
  end

  local m = mod()
  if not (m and m.get_native_shader_catalog) then
    if not nativeBridge._logOnce.catalog_fail then
      nativeBridge._logOnce.catalog_fail = true
      print("[asevoxel-native] get_native_shader_catalog not available")
    end
    return nil, "native missing"
  end

  local ok, catalog = pcall(m.get_native_shader_catalog)
  if not ok or type(catalog) ~= "table" then
    if not nativeBridge._logOnce.catalog_fail then
      nativeBridge._logOnce.catalog_fail = true
      print("[asevoxel-native] get_native_shader_catalog FAILED: " .. tostring(catalog))
    end
    return nil, catalog
  end

  nativeBridge._catalogCache = catalog
  return catalog
end
...
function nativeBridge.renderShaderStack(voxels, params)
  local stackConfig = params and params.shaderStack
  return nativeBridge.renderWithShaders(voxels, params, stackConfig)
end
