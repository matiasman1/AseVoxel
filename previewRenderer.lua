...
function previewRenderer.renderVoxelModel(model, params)
  ...
  -- Native BASIC renderer fast-path
  local canNativeBasic = nativeBridge_ok
    and nativeBridge
    and nativeBridge.isAvailable()

  local rr = _getRemote()
  if rr and rr.isEnabled and rr.isEnabled() then
    canNativeBasic = false
  end

  if canNativeBasic and model and #model > 0 then
    local flat = {}
    ...
    if nativeResult and nativeResult.pixels then
      ...
      if #bytes == expected then
        ...
        if _metrics then
          _metrics.backend = (params.shadingMode=="Stack") and "native-stack" or "native-basic"
        end
        return img
      else
...
function previewRenderer.exportOBJ(voxels, filePath, options)
  ...
  local colorFormat = options.colorFormat or "materials"

  if options.exportAtScale then
    scaleModel = options.exportAtScale
  end

  -- Compute bounds for Y inversion
  ...
  file:write("# Wavefront OBJ\n# Generated by AseVoxel Viewer\nmtllib voxel_export.mtl\n")

  local materialMap = {}
  local materialIndex = 1
  if includeColors and colorFormat == "materials" then
    local mtl = io.open(app.fs.joinPath(app.fs.filePath(filePath), "voxel_export.mtl"), "w")
    if mtl then
      mtl:write("# Materials\n")
      for _, v in ipairs(voxels) do
        local key = string.format("%d_%d_%d", v.color.r or 0, v.color.g or 0, v.color.b or 0)
        if not materialMap[key] then
          local matName = "voxel_" .. materialIndex
          materialMap[key] = matName
          materialIndex = materialIndex + 1
          mtl:write("newmtl "..matName.."\n")
          mtl:write(string.format("Kd %.6f %.6f %.6f\n",
            (v.color.r or 0)/255, (v.color.g or 0)/255, (v.color.b or 0)/255))
  ...
  for i, voxel in ipairs(voxels) do
    ...
    -- Y inversion: convert sprite Y-down to Y-up in exported space
    local invY = (heightSpan - (voxel.y - minY)) + minY
    local y = invY * scaleModel
    local z = voxel.z * scaleModel
    local s = scaleModel
    local key = string.format("%d_%d_%d", voxel.color.r or 0, voxel.color.g or 0, voxel.color.b or 0)
    if includeColors and colorFormat == "materials" and materialMap[key] then
      file:write("usemtl "..materialMap[key].."\n")
    end
    local verts = {
      {x, y, z},
      {x+s, y, z},
    ...
    file:write(string.format("f %d %d %d %d\n", base+3, base+2, base+6, base+7))   -- front
    file:write(string.format("f %d %d %d %d\n", base, base+1, base+5, base+4))     -- back
